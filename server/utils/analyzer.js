const { KNOWN_EXCHANGES } = require('./fetcher');

/**
 * Analyze address reuse patterns
 * Checks how many times the same addresses have been interacted with
 */
function analyzeAddressReuse(transactions, walletAddress) {
  ...
  // Find addresses with high reuse
 ...
 * Flags transactions with suspiciously round values
 */
function analyzeRoundNumbers(transactions) {
  if (!transactions || transactions.length === 0) {
    return {
      detected: false,
      count: 0,
  ...
  transactions.forEach((tx) => {
    ...
    // Check for round numbers (1.0, 0.5, 0.1, 10.0, etc.)
    if (valueEth > 0) {
      const isRound =
        valueEth === Math.floor(valueEth) || // Whole numbers
        valueEth * 2 === Math.floor(valueEth * 2) || // .5 increments
        valueEth * 10 === Math.floor(valueEth * 10); // .1 increments
 ...
 * Checks for direct links to known exchange addresses
 */
function analyzeExchangeLinks(transactions, walletAddress) {
  if (!transactions || transactions.length === 0) {
    return {
      detected: false,
      count: 0,
  ...
  transactions.forEach((tx) => {
    ...
    // Check if interacting with known exchanges
 ...
 * Checks if transactions occur at predictable times
 */
function analyzeTimingPatterns(transactions) {
  ...
  // Find if transactions cluster in a 3-hour window
 ...
 * Detects small incoming transactions that may be tracking attempts
 */
function analyzeDustAttacks(transactions, walletAddress) {
  if (!transactions || transactions.length === 0) {
    return {
      detected: false,
      count: 0,
  ...
  // Dust threshold: roughly $1 at typical ETH prices (0.0005 ETH)
  ...
  transactions.forEach((tx) => {
    ...
    // Check for small incoming transactions
 ...
 * Checks if there's too much predictability in transaction amounts
 */
function analyzeValueDistribution(transactions) {
  if (!transactions || transactions.length < 5) {
    return {
      detected: false,
      severity: 'good',
  ...
  const repeatedValues = Object.entries(valueCounts).filter(([_, count]) => count > 2);
  ...
  if (repeatedValues.some(([_, count]) => count > 5)) {
    severity = 'warning';
    points = -10;
    detected = true;
 ...
 */
function generateTimeline(transactions, walletAddress) {
  if (!transactions || transactions.length === 0) {
    return [];
  }

  // Group transactions by date and calculate running score
  const addressLower = walletAddress.toLowerCase();
  const dateScores = {};
  ...
  let addressesSeen = new Set();

  sorted.forEach((tx) => {
    const date = new Date(parseInt(tx.timeStamp) * 1000);
    const dateKey = date.toISOString().split('T')[0];

    // Simulate score changes based on patterns
    const to = tx.to?.toLowerCase();
    const from = tx.from?.toLowerCase();
    const otherAddress = to === addressLower ? from : to;

    if (otherAddress && addressesSeen.has(otherAddress)) {
      runningScore = Math.max(0, runningScore - 0.5); // Slight penalty for reuse
    }
...
function calculateScore(issues) {
  ...
  issues.forEach((issue) => {
    score += issue.points; // points are negative
  });
 ...
 */
function analyzeWallet(data, walletAddress) {
  ...
  const stats = {
    ...
    uniqueAddresses: addressReuse.uniqueAddresses,
    chain: data.chain,
  };
